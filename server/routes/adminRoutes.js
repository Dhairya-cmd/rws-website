...
router.post('/services', async (req, res) => {
  ...
  if (id) {
    await Service.findByIdAndUpdate(id, { title, icon, desc });
  } else {
    await new Service({ title, icon, desc, order: 0 }).save();
  }
...
router.post('/gallery', upload.array('files', 15), async (req, res) => {
  ...
  const promises = req.files.map(f => {
    const mediaUrl = `http://127.0.0.1:5000/uploads/${f.filename}`;
    return new Gallery({ category, mediaType, mediaUrl, title: "" }).save();
  });
